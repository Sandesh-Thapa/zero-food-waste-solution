...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $latest_donors = OrderDonation::orderBy('order_date','DESC')->take(4)->get();
        $order_items = [];
        ...
        foreach($latest_donors as $key=>$orderData) {
            $donorItems = OrderDonationItem::where('order_id', $orderData->id)->get();
        ...
        $doner_res = $client->request('POST', 'http://192.168.1.3:5000/requests/forecast/',['form_params'=>$data]);

        $next_doner= $doner_res->getBody()->getContents();
        $next_doner = json_decode($next_doner)->data;

        return view('dashboard', compact(['total_requests', 'total_donations', 'total_donors','next_request']));
    }
